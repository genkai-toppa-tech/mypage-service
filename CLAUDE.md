...
新機能や実装方針を検討する際は、常に「これはユーザーの積み上げ実感を可視化することに貢献するか」を判断基準にすること。

### 背景にある課題

- 日報・宣言を提出しても積み上がっている実感が湧きにくい
- Discordはチャット形式のため過去の記録が流れてしまい、振り返れない
- 志道シート・Notion資料は作って終わりになりがちで、日常的に振り返られていない

### Discordとの役割分担

Discordからの完全移行は想定していない。Discordはリアルタイムのやり取りに、マイページは日報・週報・月報の蓄積に向いている、という前提で設計すること。両者を排他的に扱わず、併用を前提にした機能設計を行う。
